...
if __name__ == '__main__':
    ...
    parser.add_argument('--dial', help='Percent of dialog-fiction in Train Set (1 to 100)', required=True)
    parser.add_argument('--nondial', help='Percent of non-dialog-fiction in Train Set (1 to 100)', required=True)
    args = parser.parse_args()

    print("Running SVM for dialog: {}% and non-dialog: {}%".format(args.dial, args.nondial))
    ...
    X, Y, pct_quoted_fic, IDs = dialog_data_loader.load_train_data(dial=int(args.dial)/100, no_dial=int(args.nondial)/100)
    
    t = [i.split('____')[0] for i in IDs]
    print("X: {} | Y: {} | Y Distribution: {} | Dialog Dist: {}".format(len(X), len(Y), Counter(Y), Counter(t)))
    print("Y preview:", Y[:3])

    results_path = '/path/Augmentation-for-Literary-Data/dialog-results/hyperparm/svm_'+str(NUMBER_OF_FOLDS)+'foldCV_dial_'+str(args.dial)+'.tsv' # name of output file
